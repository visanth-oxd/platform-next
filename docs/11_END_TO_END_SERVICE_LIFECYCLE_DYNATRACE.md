# End-to-End Service Lifecycle: Introduction to Deployment (Dynatrace-Focused)

**Status**: COMPREHENSIVE DESIGN - Complete Service Lifecycle with Dynatrace Monitoring

**Document Type**: Master Design Document

**Audience**: Platform engineers, developers, SRE teams, finance teams, observability teams

**Date**: 2025-11-16

**Note**: This is the Dynatrace-focused version. For the original version with Prometheus/Grafana, see [11_END_TO_END_SERVICE_LIFECYCLE.md](./11_END_TO_END_SERVICE_LIFECYCLE.md)

**For detailed Dynatrace monitoring design**, see [15_DYNATRACE_MONITORING_DESIGN.md](./15_DYNATRACE_MONITORING_DESIGN.md)

**Key Differences from Original**:
- ✅ Dynatrace OneAgent is the **primary** monitoring tool (no Prometheus/Grafana required)
- ✅ **Prometheus is NOT needed** - Dynatrace OneAgent collects all metrics directly from the application
- ✅ Custom metrics from separate `metrics-catalog` repository
- ✅ Base monitoring profiles + Custom metrics merged into unified Dynatrace ConfigMap
- ✅ Dynatrace Sync Controller syncs ConfigMaps to Dynatrace SaaS
- ✅ Business context: COR-B, core-banking, owner@company.com

**Important**: Dynatrace OneAgent auto-instruments your application and collects metrics directly. No Prometheus scraping or ServiceMonitor needed. Prometheus is optional only if you need long-term storage or PromQL queries.
...
Profile Expansion (Base + Custom Metrics) → Manifest Generation → 
Git Commit → Apptio Sync → Harness Deployment → 
Dynatrace Sync → Production → Monitoring Active (Dynatrace)
```
...
├─────────────────────────────────────────────────────────────────┤
│ - Schema validation                                              │
│ - Profile expansion (cost + monitoring base profiles)
│ - Custom metrics fetch (from metrics-catalog repository)
│ - Merge: Base profiles + Custom metrics                         │
...
│ - Dynatrace OneAgent (auto-instrumentation, APM)
│ - Dynatrace Sync Controller (syncs ConfigMaps to Dynatrace SaaS)                         │
│ - Cost tracking (GCP billing + Apptio)                         │
│                                                                 │
│ Note: Prometheus/Grafana optional if needed for specific use   │
│       cases, but Dynatrace is the primary monitoring tool        │
└─────────────────────────────────────────────────────────────────┘
```
...
Cost Allocation:
  Cost Center: COR-B (validated against Apptio)
...
**Form Section 4: Monitoring Configuration** ⭐ **NEW** (Dynatrace-Focused)

```yaml
Monitoring Profile: domain-api
  - Recommended for: REST APIs
  - Includes: Dynatrace OneAgent, SLOs, Alert Rules
  - Base monitoring patterns (standard metrics, alerts)
  
Dynatrace Configuration:
  ✅ Enable Dynatrace (primary monitoring tool)
  - Auto-instrumentation via OneAgent
  - Full-stack APM, distributed tracing
  - Anomaly detection
  
Custom Metrics (from metrics-catalog repository):
  Repository: https://github.com/company/metrics-catalog
  Version: v1.2.0 (pinned)
  Profiles:
    - payment-business-metrics (business-specific metrics)
...
│                                                              │
│ Monitoring:                                                  │
│ ├─ Dynatrace: ✅ Enabled (primary)                         │
│ ├─ Custom Metrics: payment-business-metrics (v1.2.0)       │
...
✅ Budgets calculated:
   ...
   Prod: $6,000/month
✅ Cost center COR-B verified in Apptio
    ...
    "alerts": [
        ...
        },
        {
            ...
            "channels": {
                ...
                "email": ["owner@company.com", "finance-operations@company.com"]
            }
        }
    ]
}
...
monitoring_profile = load_monitoring_profile("domain-api")
...
- ✅ Configures alert: 100% critical → Teams + Email
...
Developer (Backstage)
  ...
  ↓
  Cost Allocation (COR-B, owner@company.com)
  ↓
...
Developer (Backstage)
  ↓
  Monitoring Profile Selection (domain-api)
  ↓
...
- [15_DYNATRACE_MONITORING_DESIGN.md](./15_DYNATRACE_MONITORING_DESIGN.md) - **Dynatrace monitoring design (profiles + custom metrics as code)** ⭐
- [08_MONITORING_METRICS_PROFILES.md](./08_MONITORING_METRICS_PROFILES.md) - Monitoring profiles (base templates)
- [09_PROFILES_KUSTOMIZE_INTEGRATION.md](./09_PROFILES_KUSTOMIZE_INTEGRATION.md) - Technical integration
- [11_END_TO_END_SERVICE_LIFECYCLE.md](./11_END_TO_END_SERVICE_LIFECYCLE.md) - Original version (with Prometheus/Grafana)

**Note**: This document focuses on the end-to-end service lifecycle flow. For detailed Dynatrace implementation, custom metrics structure, and Kustomize examples, see [15_DYNATRACE_MONITORING_DESIGN.md](./15_DYNATRACE_MONITORING_DESIGN.md).
